...
library("limma")
library("edgeR")
edgeRUsersGuide()# User's Guide
?DGEList
y <- DGEList(counts = exp.data, group = factor(sample.data[,1]))
dim(y) # 
y$samples
head(y$samples)

keep <- rowSums(y$counts) >= 200
...
design ##treated vs untreated

##--Steep III---Data exploration
# MD plots for samples
# mean-difference (MD) plots for all the samples as a quality check, 
# An MD plot shows the log-fold change between a sample against the average expression across all the other samples. 
# This visualization can help you see if there are genes highly upregulated or downregulated in a sample. 
plotMD(y, column=1) 
...
# Generating multi-dimensional scaling (MDS) plots. 
# This visualizes the diffierences between the expression diffierences of samples in two dimensions.
?plotMDS
plotMDS(y, labels = factor(sample.data[,"condition"]), 
        col=c(rep("blue", 3), rep("black", 4))) # by Tissue

fviz_pca_ind(PCA(as.data.frame(t(y$counts))), label = "none", 
             habillage = as.data.frame(t(y$samples$group)))
...
## filter and remove rows with rowMean less than or equals to zero  
data.log <- data.log[rowMeans(data.log)>5,] ## 7176

## Farther normalization using quntitle 
quant.data.log <- normalize.quantiles(as.matrix(data.log))
colnames(quant.data.log) <- colnames(data.log) 
rownames(quant.data.log) <- rownames(data.log)

## plot
plotDensities(quant.data.log, legend = "topright", main = "log2 transformation")
boxplot(quant.data.log, main = "Log2 Boxplot_Normalized")

## center the data first  
dat <- quant.data.log - rowMeans(quant.data.log)

##MDS plot
plotMDS(dat,col= "blue", main = "MDS")
